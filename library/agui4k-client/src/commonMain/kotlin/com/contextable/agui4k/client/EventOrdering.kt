...
class EventOrderingProcessor(
    ...
    private fun getEventSequence(event: BaseEvent): Long? {
        return when (event) {
            ...
            else -> event.timestamp ?: Clock.System.now().toEpochMilliseconds() // Fallback to timestamp
        }
    }
...
fun Flow<BaseEvent>.withOrderingAndTimeout(
    ...
    var lastEventTime = Clock.System.now().toEpochMilliseconds()
    ...
    orderedEvents.collect { event ->
        ...
        if (buffer.size >= 10) {
            ...
            delay(10)
            if (Clock.System.now().toEpochMilliseconds() - lastEventTime >= 10) {
                buffer.forEach { send(it) }
                buffer.clear()
            }
        }
