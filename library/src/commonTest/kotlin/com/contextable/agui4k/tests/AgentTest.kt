...
class AgentTest {
    ...
    /**
     * Test HttpAgent configuration.
     */
    @Test
    fun testHttpAgentConfig() {
        val config = HttpAgentConfig(
            ...
            agentId = "http-agent-1",
            description = "Test HTTP Agent"
        )

        val agent = HttpAgent(config)

        assertEquals("http-agent-1", agent.agentId)
        assertEquals("Test HTTP Agent", agent.description)

        // Verify configuration
        assertEquals("https://test.example.com/agent", config.url)
