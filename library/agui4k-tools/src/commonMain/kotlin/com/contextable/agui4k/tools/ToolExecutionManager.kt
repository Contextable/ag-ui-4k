...
class ToolExecutionManager(
    ...
    private suspend fun executeToolCall(
        ...
        try {
            logger.info { "Executing tool: $toolName (ID: $toolCallId)" }
            ...
            logger.warn { "Tool not found: $toolName (ID: $toolCallId)" }
            ...
            logger.error(e) { "Tool execution failed: $toolName (ID: $toolCallId)" }
    ...
    fun cancelAllExecutions() {
        logger.info { "Cancelling ${activeExecutions.size} active tool executions" }
        activeExecutions.values.forEach { it.cancel() }
        activeExecutions.clear()
    }
