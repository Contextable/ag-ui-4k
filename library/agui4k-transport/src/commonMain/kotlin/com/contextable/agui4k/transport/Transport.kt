...
interface ClientTransport {
     ...
     */
    suspend fun startRun(message: Message, threadId: String? = null, tools: List<Tool>? = null): RunSession
     ...
     * @return A [RunSession] for managing the run lifecycle and receiving events
     */
    suspend fun startRunWithMessages(messages: List<Message>, threadId: String? = null, tools: List<Tool>? = null): RunSession {
        // Default implementation: just send the last message
        // Concrete implementations should override this to send all messages
        return startRun(messages.lastOrNull() ?: throw IllegalArgumentException("No messages provided"), threadId, tools)
    }
}